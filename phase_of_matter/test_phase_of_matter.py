...
Follows qiskit-machine-learning test conventions:
  - QiskitMachineLearningTestCase base class (falls back to unittest.TestCase
    for standalone runs outside the repo)
...
# ---------------------------------------------------------------------------
# Base class — uses QiskitMachineLearningTestCase inside the repo, plain
# unittest.TestCase when running the file standalone from the QML directory.
# When contributing, replace the try/except with:
#   from test import QiskitMachineLearningTestCase
# and update all imports below to use qiskit_machine_learning paths.
# ---------------------------------------------------------------------------
try:
    from test import QiskitMachineLearningTestCase as _Base  # type: ignore[import]
except ImportError:
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    _Base = unittest.TestCase  # type: ignore[misc,assignment]

from phase_of_matter import phase_of_matter_data  # noqa: E402
from phase_of_matter._annni import build_hamiltonian as build_annni  # noqa: E402
from phase_of_matter._base import get_ground_state_exact  # noqa: E402
from phase_of_matter._cluster import build_hamiltonian as build_cluster  # noqa: E402
from phase_of_matter._haldane import build_hamiltonian as build_haldane  # noqa: E402
from phase_of_matter._heisenberg import build_hamiltonian as build_heisenberg  # noqa: E402
...
@ddt
class TestHamiltonianBuilders(_Base):
...
class TestImportPaths(_Base):
    """Verify the package can be imported and is correctly wired up."""

    def test_importable(self):
        import phase_of_matter as pm  # pylint: disable=import-outside-toplevel

        self.assertIsNotNone(pm.phase_of_matter_data)

    def test_in_all(self):
        import phase_of_matter as pm  # pylint: disable=import-outside-toplevel

        self.assertIn("phase_of_matter_data", pm.__all__)

    def test_hamiltonian_modules_importable(self):
        from phase_of_matter import (  # pylint: disable=import-outside-toplevel
            _annni,
            _cluster,
            _haldane,
            _heisenberg,
